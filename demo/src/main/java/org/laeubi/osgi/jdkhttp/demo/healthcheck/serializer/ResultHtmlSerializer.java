 ...
 */
package org.laeubi.osgi.jdkhttp.demo.healthcheck.serializer;
...
import org.laeubi.osgi.jdkhttp.demo.healthcheck.HealthCheckParam;
...
/**
 * Adapted from
 * {@code org.apache.felix.hc.core.impl.servlet.ResultHtmlSerializer} - the
 * only change is that the parameter list is now described by
 * {@link HealthCheckParam} instead of the Servlet-only nested
 * {@code HealthCheckExecutorServlet.Param} type; the HTML rendering itself
 * has no dependency on the Servlet API.
 *
...
public class ResultHtmlSerializer {
    ...
    public String serialize(final Result overallResult, final List<HealthCheckExecutionResult> executionResults,
            List<HealthCheckParam> paramList, boolean includeDebug) {
    ...
    private String getHtmlHelpText(List<HealthCheckParam> paramList) {
        final StringBuilder sb = new StringBuilder();
        sb.append("<h3>Supported URL parameters</h3>\n");
        for (HealthCheckParam p : paramList) {
            sb.append("<b>").append(p.getName()).append("</b>:");
            sb.append(escapeHtml(p.getDescription()));
            sb.append("<br/>");
        }
        return sb.toString();
    }
