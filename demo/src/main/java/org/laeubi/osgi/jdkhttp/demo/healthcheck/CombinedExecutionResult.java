 ...
 * {@code org.apache.felix.hc.core.impl.executor.CombinedExecutionResult} - it
 * has no dependency on the Servlet API and is only package-private ({@code impl})
 * in the original bundle, so it is not usable from outside; reproduced here
 * to combine several {@link HealthCheckExecutionResult}s into a single
 * overall {@link Result}.
 */
class CombinedExecutionResult implements HealthCheckExecutionResult {

    final List<HealthCheckExecutionResult> executionResults;
    final Result overallResult;

    CombinedExecutionResult(List<HealthCheckExecutionResult> executionResults) {
        this(executionResults, Result.Status.OK);
    }

    CombinedExecutionResult(List<HealthCheckExecutionResult> executionResults, Result.Status statusForZeroResult) {
